...
export const useUrlBoolState = (initialValue: boolean, key: string) => {
    ...
    const [state, setCurrentState] = useState(urlValue ? urlValue : serializeBool(initialValue));
    ...
    useEffect(() => {
        if (urlValue && urlValue !== state)
            setState(deserializeBool(urlValue))
        if (!urlValue && initialValue)

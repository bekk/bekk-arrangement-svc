...
let private participationsToAttendeesAndWaitlist participations =
    let participations = Seq.toList participations
    { Attendees = List.filter (fun p -> not p.Participant.IsWaitlisted) participations
      WaitingList = List.filter (fun p -> p.Participant.IsWaitlisted) participations }
...
let private participateEvent isBekker numberOfParticipants (event: Models.Event) =
    ...
    let hasRoom =
        event.MaxParticipants.IsNone
        || event.MaxParticipants.IsSome
           && event.MaxParticipants.Value > numberOfParticipants
    ...
    // Eventet er ikke åpent for registrering
        ...
        event.OpenForRegistrationTime >= currentEpoch
        || (event.CloseRegistrationTime.IsSome
            && event.CloseRegistrationTime.Value < currentEpoch)
    then
        NotOpenForRegistration
    ...
    fun (next: HttpFunc) (context: HttpContext) ->
        let result =
            taskResult {
                ...
                let! writeModel = decode ParticipantWriteModel.decoder context |> TaskResult.mapError BadRequest

                let config = context.GetService<AppConfig>()

                use db = openTransaction context
                let! eventAndQuestions = Queries.getEvent eventId db |> TaskResult.mapError InternalError
                let! eventAndQuestions = eventAndQuestions |> Result.requireSome (eventNotFound eventId)

                do!
                    (isBekker || eventAndQuestions.Event.IsExternal)
                    |> Result.requireTrue mustBeAuthorizedOrEventMustBeExternal

                let numberOfParticipants =
                    Option.defaultValue 0 eventAndQuestions.NumberOfParticipants
                ...
                let! _ =
                    ...
                    | NotExternal -> Error "Arrangementet er ikke eksternt"
                    | IsCancelled -> Error "Arrangementet er kansellert"
                    | NotOpenForRegistration -> Error "Arrangementet er ikke åpent for registering"
                    | HasAlreadyTakenPlace -> Error "Arrangementet tok sted i fortiden"
                    | NoRoom -> Error "Arrangementet har ikke plass"
                    | IsWaitListed
                    | CanParticipate -> Ok()
                ...
                let! participant =
                    Queries.addParticipantToEvent
                        eventId
                        email
                        userId
                        writeModel.Name
                        writeModel.Department
                        isWaitlisted
                        db
    ...
    fun (next: HttpFunc) (context: HttpContext) ->
        let result =
            taskResult {
                ...
                let! events = Queries.getEventsForForside email db |> TaskResult.mapError InternalError
                return events |> Seq.map Event.encodeForside |> Encode.seq
            }
...
let getFutureEvents (next: HttpFunc) (context: HttpContext) =
    let result =
        taskResult {
            let! userId = getUserId context |> Result.requireSome couldNotRetrieveUserId
            use db = openConnection context
            let! eventAndQuestions = Queries.getFutureEvents userId db |> TaskResult.mapError InternalError
            let result = eventAndQuestions |> List.map Event.encodeEventAndQuestions
            return result
        }
...
let getEventsSummary =
    fun (next: HttpFunc) (context: HttpContext) ->
        let result =
            taskResult {
                use db = openConnection context
                let! events = Queries.getEventsSummary db |> TaskResult.mapError InternalError
                return events |> Seq.map Event.encodeSummary |> Encode.seq
            }
...
let getPublicEvents =
    fun (next: HttpFunc) (context: HttpContext) ->
        let result =
            taskResult {
                use db = openConnection context
                let! events = Queries.getPublicEvents db |> TaskResult.mapError InternalError
                return events |> Seq.map Event.encodeSummary |> Encode.seq
            }
...
let getPastEvents (next: HttpFunc) (context: HttpContext) =
    let result =
        taskResult {
            let! userId = getUserId context |> Result.requireSome couldNotRetrieveUserId
            use db = openConnection context
            let! eventAndQuestions = Queries.getPastEvents userId db |> TaskResult.mapError InternalError
            return eventAndQuestions |> List.map Event.encodeEventAndQuestions |> Encode.list
        }
...
let getEventsOrganizedBy (email: string) =
    fun (next: HttpFunc) (context: HttpContext) ->
        let result =
            taskResult {
                use db = openConnection context
                let! eventAndQuestions = Queries.getEventsOrganizedByEmail email db |> TaskResult.mapError InternalError
                return eventAndQuestions |> List.map Event.encodeEventAndQuestions
            }
...
let getEventIdByShortnameHttpResult shortname db =
    taskResult {
        let! result = Queries.getEventIdByShortname shortname db |> TaskResult.mapError InternalError
...
let getEventIdByShortname =
    fun (next: HttpFunc) (context: HttpContext) ->
        let result =
            taskResult {
                let! shortnameEncoded = context.GetQueryStringValue "shortname" |> Result.mapError BadRequest
...
let getEvent (eventId: Guid) =
    fun (next: HttpFunc) (context: HttpContext) ->
        ...
        let result =
            taskResult {
                use db = openConnection context
                let! isEventExternal = Queries.isEventExternal eventId db |> TaskResult.mapError InternalError

                do!
                    (isBekker || isEventExternal)
                    |> Result.requireTrue mustBeAuthorizedOrEventMustBeExternal
...
let getUnfurlEvent (idOrName: string) =
    fun (next: HttpFunc) (context: HttpContext) ->
        let strSkip n (s: string) =
            s |> Seq.safeSkip n |> Seq.map string |> String.concat ""
        ...
        let result =
            taskResult {
                ...
                return
                    {| event = Event.encodeEventAndQuestions eventAndQuestions
...
let createEvent =
    fun (next: HttpFunc) (context: HttpContext) ->
        let result =
            taskResult {
                let! writeModel = decode EventWriteModel.decoder context |> TaskResult.mapError BadRequest
                let! userId = getUserId context |> Result.requireSome couldNotRetrieveUserId
                use db = openTransaction context
                ...
                let! newEvent = Queries.createEvent writeModel userId db |> TaskResult.mapError InternalError
    ...
    fun (next: HttpFunc) (context: HttpContext) ->
        let result =
            taskResult {
                ...
                do! Queries.cancelEvent eventId db |> TaskResult.mapError InternalError
                let! eventAndQuestions = Queries.getEvent eventId db |> TaskResult.mapError InternalError
                let! eventAndQuestions = eventAndQuestions |> Result.requireSome (eventNotFound eventId)
                let! participants = Queries.getParticipantsForEvent eventId db |> TaskResult.mapError InternalError
                db.Commit()
    ...
    fun (next: HttpFunc) (context: HttpContext) ->
        let result =
            taskResult {
                ...
                let! eventAndQuestions = eventAndQuestions |> Result.requireSome (eventNotFound eventId)
                let! participants = Queries.getParticipantsForEvent eventId db |> TaskResult.mapError InternalError
                do! Queries.deleteEvent eventId db |> TaskResult.mapError InternalError
                db.Commit()
                let! messageToParticipants = context.ReadBodyFromRequestAsync()
    ...
    fun (next: HttpFunc) (context: HttpContext) ->
        let result =
            taskResult {
                let! userId = getUserId context |> Result.requireSome couldNotRetrieveUserId
                let userIsAdmin = isAdmin context
                do! (userId = id || userIsAdmin) |> Result.requireTrue cannotSeeParticipations
                use db = openConnection context
                let! eventsAndQuestions = Queries.getEventsOrganizedById id db |> TaskResult.mapError InternalError
                let! participations = Queries.getParticipationsById id db |> TaskResult.mapError InternalError
                return Participant.encodeWithLocalStorage eventsAndQuestions (participations |> Seq.toList)
            }

        jsonResult result next context

let private canUpdateNumberOfParticipants
    (oldEvent: Models.Event)
    (newEvent: Models.EventWriteModel)
    ...
    | Some oldMax, Some newMax ->
        ...
        // Det er plass til alle
            ...
            let isRemovingWaitlist =
                newEvent.HasWaitingList = false && oldEvent.HasWaitingList = true
...
let private canUpdateQuestions
    (newEventQuestions: ParticipantQuestionWriteModel list)
    (oldEventQuestions: ParticipantQuestion list)
    oldEventParticipants
    =
    let newEventQuestions =
        newEventQuestions |> List.map (fun question -> question.Question) |> List.sort

    let oldEventQuestions =
        oldEventQuestions |> List.map (fun question -> question.Question) |> List.sort
    ...
    {| ShouldChangeQuestions = shouldChangeQuestions
       Error = shouldChangeQuestions && not canChangeQuestions |}

let private sendEmailToNewParticipants
    oldEventMaxParticipants
    newEventMaxParticipants
    (oldEventParticipants: Models.Participant seq)
    updatedEvent
    context
    =
    let oldEventWaitlist = Seq.filter (fun p -> p.IsWaitlisted) oldEventParticipants
    ...
    if numberOfPeople > 0 then
        let newPeople = oldEventWaitlist |> Seq.truncate numberOfPeople
...
let sendUpdateEmailToOldParticipants
    (old': Event)
    (new': Event)
    (oldParticipants: Participant seq)
    ...
    |> Option.iter (fun diff ->
        ...
        |> Seq.map (fun (p: Participant) ->
            let diffMsg =
                let timeChanged =
                    DateTimeCustom.toReadableDiff diff.oldStart diff.oldEnd diff.newStart diff.newEnd

                let locationChanged =
                    if diff.oldLocation <> diff.newLocation then
                        Some(diff.oldLocation, diff.newLocation)
                    else
                        None
                ...
                [ $"Hei! 😄"
                  ...
                  | None, None ->
                      failwithf
                          "Only expected to send update notification for certain fields, but there are other changes!"
                  $""
                  $"Vi gleder oss til å se deg! 🎉"
                  $""
                  if new'.MaxParticipants.IsSome then
                      ...
                      "Gjerne meld deg av dersom du ikke lenger har mulighet til å delta."
    ...
    fun (next: HttpFunc) (context: HttpContext) ->
        let result =
            taskResult {
                ...
                do! canEditEvent |> Result.requireTrue cannotUpdateEvent
                let! oldEvent = Queries.getEvent eventId db |> TaskResult.mapError InternalError

                let! oldEventParticipants =
                    Queries.getParticipantsForEvent eventId db |> TaskResult.mapError InternalError

                let! oldEvent = oldEvent |> Result.requireSome (eventNotFound eventId)

                let numberOfParticipantsForOldEvent =
                    Option.defaultValue 0 oldEvent.NumberOfParticipants

                do!
                    canUpdateNumberOfParticipants oldEvent.Event writeModel numberOfParticipantsForOldEvent
                    |> Result.mapError id

                let canUpdateQuestions =
                    canUpdateQuestions
                        writeModel.ParticipantQuestions
                        oldEvent.Questions
                        numberOfParticipantsForOldEvent
                ...
                if canUpdateQuestions.Error then
                    ...
                    let! updatedEvent = Queries.updateEvent eventId writeModel db |> TaskResult.mapError InternalError
                    db.Commit()
    ...
    fun (next: HttpFunc) (context: HttpContext) ->
        let result =
            taskResult {
                ...
                return Participant.encodeParticipationsAndWaitlist (participationsToAttendeesAndWaitlist participations)
            }
...
let createCsvString
    ...
    (participants: ParticipationsAndWaitlist)
    =
    ...
    let createParticipant (builder: StringBuilder) (participantAndAnswers: ParticipantAndAnswers) =
        ...
        let employeeId =
            participant.EmployeeId |> Option.map string |> Option.defaultValue ""

        let department = participant.Department |> Option.defaultValue ""
    ...
    let questions =
        questions |> List.map (fun q -> formatString q.Question) |> String.concat ","
...
let addUtf8BomToCsv (csvString: string) =
    Array.concat [ Encoding.UTF8.GetPreamble(); Encoding.UTF8.GetBytes(csvString) ]
    ...
    fun (next: HttpFunc) (context: HttpContext) ->
        let result =
            taskResult {
                ...
                let participants = participationsToAttendeesAndWaitlist participations

                let csvString =
                    createCsvString eventAndQuestions.Event eventAndQuestions.Questions participants
    ...
    fun (next: HttpFunc) (context: HttpContext) ->
        let result =
            taskResult {
                ...
                let attendeesAndWaitlist = participationsToAttendeesAndWaitlist participations
    ...
    fun (next: HttpFunc) (context: HttpContext) ->
        let result =
            taskResult {
                ...
                return result |> Seq.map Participant.encodeParticipantAndAnswers
            }
    ...
    fun (next: HttpFunc) (context: HttpContext) ->
        let result =
            taskResult {
                ...
                let! participant = participant |> Result.requireSome (participantNotFound email eventId)
                let! eventAndQuestions = Queries.getEvent eventId db |> TaskResult.mapError InternalError
                let! eventAndQuestions = eventAndQuestions |> Result.requireSome (eventNotFound eventId)
                ...
                do!
                    (isAdmin
                     ...
                     || (cancellationToken.IsSome
                         && cancellationToken.Value = participant.CancellationToken))
                ...
                let participationsAndWaitlist = participationsToAttendeesAndWaitlist participants
                ...
                sendParticipantCancelMails
                    eventAndQuestions.Event
                    deletedParticipant
                    deletedParticipantAnswers
                    personWhoGotIt
                    context
          ...
          >=> choose
              [ route "/api/events/id" >=> getEventIdByShortname
                ...
                routef "/api/events/%O/participants/%s/waitinglist-spot" (fun (eventId, email) ->
                    getWaitinglistSpot eventId email)

...
module Helpers =
    let addTimeSpan<'a> (tm: TimeSpan) (o: 'a) : 'a =
        match o :> obj with
        | :? DateTime as x -> x.Add(tm) :> obj :?> 'a
        | :? TimeSpan as x -> x.Add(tm) :> obj :?> 'a
...
type UpdateEvent() =
    ...
    let makeTime (h: int) (m: int) =
        let now = DateTime.Now in DateTime(now.Year, now.Month, now.Day, h, m, 0)
    ...
    let run
        (mapOldValue: DateTime * TimeSpan -> DateTime * TimeSpan)
        (mapNewValue: DateTime * TimeSpan -> DateTime * TimeSpan)
        =
        let now = DateTime.Now.Date
        let duration = TimeSpan.Zero

        let oldStart, oldDuration =
            mapOldValue (now, duration)

        let oldEnd = oldStart + oldDuration

        let newStart, newDuration =
            mapNewValue (oldStart, oldDuration)
    ...
    member _.``Change start time, zero duration``() =
        let actual =
            run (alwaysStart (makeTime 15 5)) (mapStart ((addHours 1) >> (addMinutes 1)))
    ...
    member _.``Change duration from zero to non-zero``() =
        let actual =
            run (alwaysStart (makeTime 15 5)) (mapDuration (addHours 1))
    ...
    member _.``Change duration in days``() =
        let actual =
            run (always (DateTime(2020, 1, 2, 3, 4, 5), TimeSpan.Zero)) (mapDuration (addDays 1))
    ...
    member _.``Change date``() =
        let actual =
            run (alwaysStart (DateTime(2020, 1, 2, 3, 4, 5))) (mapStart (addDays 1))
    ...
    member _.``Change date and start``() =
        let actual =
            run (alwaysStart (DateTime(2020, 1, 2, 3, 4, 5))) (mapStart ((addDays 1) >> (addHours 1)))
    ...
    member _.``Change date and duration from zero to non-zero``() =
        let actual =
            run
                (alwaysStart (DateTime(2020, 1, 2, 3, 4, 5)))
                ((mapStart (addDays 1))
                 >> (mapDuration (addHours 1)))
    ...
    member _.``Change date and duration from non-zero to zero``() =
        let actual =
            run
                (always (DateTime(2020, 1, 2, 3, 4, 5), TimeSpan.FromHours(1)))
                ((mapStart (addDays 1))
                 >> (alwaysDuration TimeSpan.Zero))
    ...
    member _.``Change everything``() =
        let actual =
            run
                (alwaysStart (DateTime(2020, 1, 2, 3, 4, 5))
                 >> (mapDuration ((addDays 1) >> (addHours 1))))
                ((mapStart ((addDays 1) >> (addHours 1)))
                 >> (mapDuration ((addDays 1) >> (addHours 1))))
